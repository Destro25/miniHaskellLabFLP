...
sugarVar (IndexedVar x ind) 
 |ind > 0 = Var (x ++ "_" ++ (show ind)) 
 |otherwise = Var x
...
desugarExp (Let var exp1 exp2) = App (Lam (desugarVar var) (desugarExp exp1)) (desugarExp exp2)
desugarExp (LetRec var exp1 exp2) = App (Lam (desugarVar var) (desugarExp exp2)) (App fixExp (Lam (desugarVar var) (desugarExp exp1)))
...
sugarExp (X exp) = CX (sugarVar exp)
sugarExp (Lam var exp) = CLam (sugarVar var) (sugarExp exp)
